(ns dk.ative.docjure.spreadsheet-test
  ...
  (:import (org.apache.poi.ss.usermodel Workbook Sheet Cell Row CellStyle IndexedColors Font CellValue)
           (org.apache.poi.xssf.usermodel XSSFWorkbook XSSFFont XSSFDataValidation)
...
(deftest add-validation!-test
  (let [sheet-name "Sheet 1"
        ...
        workbook-with (create-workbook sheet-name validation-map sheet-data)
        workbook-without (create-workbook sheet-name sheet-data)]
    (testing "With validation"
      ...
      (testing "Validation creation"
        (let [validation (.. workbook-with (getSheetAt 0) (getDataValidations) (get 0))
              formula (read-string (.. validation (getValidationConstraint) (getFormula1)))
              formula-vec (apply str (interpose "," formula-vec))
              cell-range (.. validation (getRegions) (getCellRangeAddresses))]
          (is (= XSSFDataValidation (class validation)))
